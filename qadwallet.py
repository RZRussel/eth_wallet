...
def get_balance(addr):
    ...
    response = requests.post("http://"+K_NODE_HOST+":"+K_NODE_PORT, data=req_json)
...
def send(priv, to, value):
    addr = utils.privtoaddr(priv)
    nonce = get_tx_count(addr)
    gasprice = get_tx_gasprice()

    trans = transactions.Transaction(nonce, gasprice, K_SEND_TX_STARTGAS, to, value, b'')
...
def usage():
    ...
    usage_str = usage_str + "\tpython qadwallet.py --key <hex private key>\n\n"
    usage_str = usage_str + "Sends value in Wei to the given account\n"
    usage_str = usage_str + """\tpython qadwallet.py --key <hex private key> --to <hex recipient address>\
 --value <wei value>\n\n"""
    usage_str = usage_str + "Provides transaction's status\n"
    usage_str = usage_str + "\tpython qadwallet.py --tx <tx hex hash>\n\n"
